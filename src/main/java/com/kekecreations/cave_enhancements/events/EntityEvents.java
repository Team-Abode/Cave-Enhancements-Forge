...
public class EntityEvents {
    ...
    public static void onEntityHurt(LivingHurtEvent event) {
        ...
        if (attacker instanceof LivingEntity livingAttacker && livingAttacker.hasEffect(ModEffects.REVERSAL.get())) {
            if (dataManager.getValue(CaveEnhancements.REVERSAL_DAMAGE) > 0) {
                attacker.level.playSound(null, attacker.blockPosition(), ModSounds.EFFECT_REVERSAL_REVERSE.get(), SoundSource.PLAYERS, 0.75F, 1.0F);
            }
