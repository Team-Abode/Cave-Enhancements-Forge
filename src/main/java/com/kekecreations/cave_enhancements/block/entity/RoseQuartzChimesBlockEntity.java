    ...
    public static void tick(Level world, BlockPos pos, RoseQuartzChimesBlockEntity entity) {
        ...
        for (LivingEntity livingEntity : list) {
            ...
            if ((otherEntity instanceof AgeableMob || otherEntity instanceof Player)) {
                if (world.isRaining() && entity.ticksTillActivateClear <= 600 && !otherEntity.hasEffect(ModEffects.EASING.get()) ) {
                    otherEntity.addEffect(new MobEffectInstance(ModEffects.EASING.get(), 300, 1, false, true));
                } else if (!world.isRaining() && entity.ticksTillActivateClear <= 0 && !otherEntity.hasEffect(ModEffects.EASING.get())) {
                    otherEntity.addEffect(new MobEffectInstance(ModEffects.EASING.get(), 300, 0, false, true));
                }
            }
        }
        ...
        if ((entity.ticksTillActivateClear <= 600 && world.isRaining()) || (entity.ticksTillActivateClear <= 0 && !world.isRaining())) {
            ...
            world.addParticle(ModParticles.ROSE_CHIMES.get(), entity.getBlockPos().getX() + 0.5D, entity.getBlockPos().getY() + 0.3D, entity.getBlockPos().getZ() + 0.5D, 0D, 0D, 0D);
