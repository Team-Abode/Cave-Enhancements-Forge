...
public class ChargedLightningAnchorBlock extends Block {
    public ChargedLightningAnchorBlock(Properties settings){
        super(settings);
    }
    ...
    private void activate(Level world, BlockPos pos, boolean interact){
        ...
        if((powered || interact)) {
            ...
            if (!world.isClientSide) {
                ((ServerLevel)world).sendParticles(ModParticles.SHOCKWAVE.get(), pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            }


            world.setBlockAndUpdate(pos, ModBlocks.LIGHTNING_ANCHOR.get().defaultBlockState());
